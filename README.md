...
- Tool rail buttons select a **category only**. Choose a variant in Tool Options, then tap the canvas to place it; selecting Panel, Bubble, Text or Effects never inserts an object by itself.
- **Select** mode taps objects and uses Fabric handles for move, resize and rotate. Empty canvas areas retain native one-finger scrolling.
- **Hand** mode disables object interaction so one-finger horizontal/vertical scrolling navigates the long episode without a keyboard.
- Two-finger pinch changes editor zoom and two-finger movement pans the scrollable workspace. The workspace uses momentum scrolling in iPad Safari.
- During placement, use the visible **Cancel** button (or `Escape` with a keyboard). Enable **Keep adding** to place repeated variants.
- At tablet widths, Tool Options and Layers/Properties become drawers. The menu and layers buttons in the top toolbar reopen them in portrait or landscape orientation.
- Panel Properties includes live corner radius (set `0` for sharp corners), fill opacity, border visibility/style, aspect lock, colors and presets.
- Focus, speed/motion, gradient and vignette effects retain procedural configuration in the project. Selecting an effect exposes its geometry controls and regenerates its vector children live.
...
| `Space` + drag | Pan |
| `Ctrl/Cmd` + wheel | Zoom |
| `Escape` | Cancel object placement |
...
- `tests/e2e/ipad-editor.spec.ts` – Playwright placement, effect, scrolling and tablet-drawer coverage

## Limitations

This release uses a continuous Fabric surface rather than visual section virtualization; exported rendering is segmented, but editing extremely dense episodes can still depend on device GPU memory. Panel-image masking, arbitrary crop handles, draggable bubble-tail endpoints, multi-selection grouping/ungrouping and custom font upload remain recommended follow-ups. Bubble text can be edited after ungrouping through Fabric internals, while free text has direct editing controls.
